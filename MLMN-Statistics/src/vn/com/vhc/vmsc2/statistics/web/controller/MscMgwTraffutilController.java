...
import vn.com.vhc.vmsc2.statistics.dao.DyMscMgwTraffutilDAO;
import vn.com.vhc.vmsc2.statistics.dao.HrMscMgwTraffutilDAO;
...
@Controller
@RequestMapping("/report/core/traffic-utilization/*")
public class MscMgwTraffutilController extends BaseController{
	@Autowired
	private HrMscMgwTraffutilDAO hrMsctraffDAO;
	@Autowired
	private DyMscMgwTraffutilDAO dyMsctraffDAO;
	@Autowired
	...
	public String Report(@RequestParam(required = false) String mscid, @RequestParam(required = false) String startHour,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				List<HrMscMgwTraffutil> HrMscTraff = hrMsctraffDAO.filter(startHour,startDate,endHour,endDate,mscid);
				
				if(endDate.compareTo(startDate) < 0)
				{
					startDate = new Date();
					HrMscTraff = hrMsctraffDAO.filter2(startHour,startDate,endHour,mscid);
				}
				...
				model.addAttribute("HrMscTraff", HrMscTraff);
				model.addAttribute("hourList", hourList);
				return "hrMscMgwTraffutil";
			}
	...
	public String showReport(@RequestParam(required = false) String mscid,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				List<DyMscMgwTraffutil> DyMscTraff = dyMsctraffDAO.filter(mscid,df.format(startDate), df.format(endDate));
				model.addAttribute("DyMscTraff", DyMscTraff);
