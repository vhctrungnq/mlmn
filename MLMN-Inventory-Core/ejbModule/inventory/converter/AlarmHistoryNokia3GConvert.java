...
// Su dung de convert file du lieu History Nokia 3G
public class AlarmHistoryNokia3GConvert extends IN_BasicConverter {
	private static final Logger logger = Logger.getLogger(AlarmHistoryNokia3GConvert.class.getName()); 
	private String sep = ";";
	private String ne = "", bts ="", cell ="",alarmType = "", fmAlarmId = "", severity = "", sdate = "",
			 edate="",alarmId = "", alarmName = "", alarmInfo = "", day = "";
	@Override
	public void convertFile(File file, String direcPath, Hashtable<Byte, String> params) throws IN_ConvertException {
		...
		try {
			...
			while ((strLine = reader.readLine()) != null){
				...
				if(count == 5){
					wAlarmLog.write("#USER_NAME;TYPE;SDATE;EDATE;VENDOR;NETWORK;NE_TYPE;NE;SITE;CELLID;SEVERITY;FM_ALARM_ID;ALARM_ID;ALARM_NAME;ALARM_INFO\n");
					blockRsc = true;  
				}
				...
				if(blockRsc){
					if(blockAlarm){
						if(strLine.contains(ne) || strLine.contains("NUMBER OF ALARM EVENTS DISPLAYED")){
					...
					if(blockAlarm){
						...
						if(countBlock == 1){ 
							try {
								fmAlarmId = str[1].substring(str[1].indexOf("(")+1, str[1].indexOf(")"));
							} catch (Exception e) {
