...
import vn.com.vhc.vmsc2.statistics.dao.VRpDyNBscLoad3gDAO;
 ...
 */
@Controller
@RequestMapping("/report/core/bsc-load-nsn-3g/*")
public class BscLoadNsn3gController extends BaseController {
	@Autowired
	private VRpDyNBscLoad3gDAO bscLoadDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String bscid, @RequestParam(required = false) String startHour,
		...
		bscid = ModelAddtributes.checkRnc(model, listBsc, bscid,"bsc-load-nsn-3g");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDyNBscLoad3g> dyBscList = bscLoadDAO.filterHourly("V_RP_HR_N_BSC_LOAD_3G", bscid, "", s[0], s[1], s[2]);
	...
	public String showReport(@RequestParam(required = false) String bscid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDyNBscLoad3g> dyBscList = bscLoadDAO.filterDaily("V_RP_DY_N_BSC_LOAD_3G", bscid, "", s[0], s[1]);
		model.addAttribute("dyBscList", dyBscList);
