...
<c:choose>
 	<c:when test="${function=='accessibility' }">
 		<content tag="heading">CELL ${cellid} ACCESSIBILITY BH WEEKLY REPORT</content>
 	</c:when>
 	<c:when test="${function=='mobility' }">
  	 	<content tag="heading">CELL ${cellid} MOBILITY BH WEEKLY REPORT</content>	
 	</c:when>
 	<c:when test="${function=='retainability' }">
  		<content tag="heading">CELL ${cellid} RETAINABILITY BH WEEKLY REPORT</content>	
 	</c:when>
 	<c:when test="${function=='traffic' }">
 	 	<content tag="heading">CELL ${cellid} TRAFFIC BH WEEKLY REPORT</content>	
 	</c:when>
 	<c:otherwise>
 		<content tag="heading">CELL ${cellid} QOS BH WEEKLY REPORT</content>	
 	</c:otherwise>
</c:choose>
<ul class="ui-tabs-nav">
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio3g/cell/wk/bhDetails.htm?function=${function}&cellid=${cellid}&bscid=${bscid}&endWeek=${endWeek}&endYear=${endYear}"><span>Báo cáo BH tuần</span></a></li>
  <li class=""><a href="${pageContext.request.contextPath}/report/radio3g/cell/mn/bhDetails.htm?cellid=${cellid}&function=${function}&bscid=${bscid}"><span>Báo cáo BH tháng</span></a></li>
</ul>
