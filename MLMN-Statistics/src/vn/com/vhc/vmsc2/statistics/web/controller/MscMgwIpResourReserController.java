...
import vn.com.vhc.vmsc2.statistics.dao.DyMscMgwIpResourReserDAO;
import vn.com.vhc.vmsc2.statistics.dao.HrMscMgwIpResourReserDAO;
...
@Controller
@RequestMapping("/report/core/resour-reservation/*")
public class MscMgwIpResourReserController extends BaseController{
	@Autowired
	private HrMscMgwIpResourReserDAO HrMscIpResourDAO;
	@Autowired
	private DyMscMgwIpResourReserDAO DyMscIpResourDAO;
	@Autowired
	...
	public String ShowReport(@RequestParam(required = true) String mscid, @RequestParam(required = false) String startHour,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				List<HrMscMgwIpResourReser> HrMscIpResour = HrMscIpResourDAO.filter(startHour,startDate,endHour,endDate,mscid);
				
				if(endDate.compareTo(startDate) < 0)
				{
					startDate = new Date();
					HrMscIpResour = HrMscIpResourDAO.filter2(startHour,startDate,endHour,mscid);
				}
				...
				model.addAttribute("HrMscIpResour", HrMscIpResour);
				model.addAttribute("hourList", hourList);
				return "hrMSCMgwIpResourReser";
			}
	...
	public String showReport(@RequestParam(required = true) String mscid,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				model.addAttribute("endDate", df.format(endDate));
				model.addAttribute("mscid", mscid);
				List<DyMscMgwIpResourReser> DyMscIpResour = DyMscIpResourDAO.filter(mscid,df.format(startDate), df.format(endDate));
				model.addAttribute("DyMscIpResour", DyMscIpResour);
				MscFilter h_msc = new MscFilter();
				h_msc.setType("MGW");
				List<Msc> mscList = mscDAO.filter(h_msc);
				model.addAttribute("mscid", mscid);
				model.addAttribute("mscList", mscList);
				return "dyMSCMgwIpResourReser";
			}
}
