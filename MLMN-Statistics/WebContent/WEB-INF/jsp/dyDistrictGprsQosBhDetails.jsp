...
<title>district gprs qos daily report</title>
<content tag="heading">GPRS QOS DISTRICT ${district}/${province} REPORT</content>
<ul class="ui-tabs-nav">
  <li class=""><a href="${pageContext.request.contextPath}/report/radio/district-gprs-qos/hr/details.htm?province=${province}&district=${district}&day=${endDate}"><span>Báo cáo giờ</span></a></li>
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/district-gprs-qos/dy/bhDetails.htm?province=${province}&district=${district}&endDate=${endDate}"><span>Báo cáo BH ngày</span></a></li>
...
<display:table name="${vRpDyDistrictGprsQosBhDetails}" id="vRpDyDistrictGprsQosBhDetail" requestURI="" pagesize="100" class="simple2" export="true">
		<display:column property="day" format="{0,date,dd/MM/yyyy}" titleKey="DAY"/>
	    <display:column titleKey="Busy Hour">
	    	${vRpDyDistrictGprsQosBhDetail.bh}:00
	    </display:column>
    	...
    	<a href="${pageContext.request.contextPath}/report/radio/province-gprs-qos/hr/details.htm?province=${vRpDyDistrictGprsQosBhDetail.province}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyDistrictGprsQosBhDetail.day}"/>">${vRpDyDistrictGprsQosBhDetail.province}</a>&nbsp;
    	</display:column>
    	...
    	<a href="${pageContext.request.contextPath}/report/radio/district-gprs-qos/hr/details.htm?province=${vRpDyDistrictGprsQosBhDetail.province}&district=${vRpDyDistrictGprsQosBhDetail.district}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyDistrictGprsQosBhDetail.day}"/>">${vRpDyDistrictGprsQosBhDetail.district}</a>&nbsp;
    	</display:column>  
	...
	$(function() {
		...
		$( '.checkAll' ).live( 'change', function() {
			...
			if($( this ).is( ':checked' )){
				for (var i=8;i<=15;i++)
				{
					$('#vRpDyDistrictGprsQosBhDetail td:nth-child('+i+'),#vRpDyDistrictGprsQosBhDetail th:nth-child('+i+')').show();
				}
			} else {
				for (var i=8;i<=15;i++)
				{
					$('#vRpDyDistrictGprsQosBhDetail td:nth-child('+i+'),#vRpDyDistrictGprsQosBhDetail th:nth-child('+i+')').hide();
				}
			}
		});
