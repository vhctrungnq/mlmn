...
<content tag="heading">HANDOVER BSC ${bscid} REPORT</content>
<ul class="ui-tabs-nav">
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/bsc-ho/dy/bhDetails.htm?bscid=${bscid}&endDate=${endDate}"><span>Báo cáo BH ngày</span></a></li>
...
<display:table name="${vRpDyBscHoBhDetails}" id="vRpDyBscHoBhDetail" requestURI="" pagesize="100" class="simple2" export="true">
		<display:column property="region" titleKey="TT"/>
	    <display:column property="day" format="{0,date,dd/MM/yyyy}" titleKey="DAY"/>
	    <display:column titleKey="BUSY HOUR">
			${vRpDyBscHoBhDetail.bh}:00
		</display:column>
	    <display:column property="bscid" titleKey="BSCID" headerClass="hide" class="hide"/>
	    <display:column titleKey="BSC" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/bsc-ho/hr/details.htm?bscid=${vRpDyBscHoBhDetail.bscid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyBscHoBhDetail.day}"/>">${vRpDyBscHoBhDetail.bscid}</a>
	    </display:column> 
	...
	$(function() {
		...
		$( '.checkAll' ).live( 'change', function() {
			...
			if($( this ).is( ':checked' )){
				for (var i=6;i<=12;i++)
				{
					$('#vRpDyBscHoBhDetail td:nth-child('+i+'),#vRpDyBscHoBhDetail th:nth-child('+i+')').show();
				}
			} else {
				for (var i=6;i<12;i++)
				{
					$('#vRpDyBscHoBhDetail td:nth-child('+i+'),#vRpDyBscHoBhDetail th:nth-child('+i+')').hide();
				}
			}
		});
