...
<c:choose>
 	<c:when test="${function=='accessibility' }">
 		<content tag="heading">CELL ${cellid} ACCESSIBILITY WEEKLY REPORT</content>
 	</c:when>
 	<c:when test="${function=='mobility' }">
  	 	<content tag="heading">CELL ${cellid} MOBILITY WEEKLY REPORT</content>	
 	</c:when>
 	<c:when test="${function=='retainability' }">
  		<content tag="heading">CELL ${cellid} RETAINABILITY WEEKLY REPORT</content>	
 	</c:when>
 	<c:when test="${function=='traffic' }">
 	 	<content tag="heading">CELL ${cellid} TRAFFIC WEEKLY REPORT</content>	
 	</c:when>
 	<c:otherwise>
 		<content tag="heading">CELL ${cellid} QOS WEEKLY REPORT</content>	
 	</c:otherwise>
</c:choose>
<ul class="ui-tabs-nav">
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio3g/cell/wk/detail.htm?function=${function}&cellid=${cellid}&bscid=${bscid }&endWeek=${endWeek}&endYear=${endYear}"><span>Báo cáo tuần</span></a></li>
  ...
  <li class=""><a href="${pageContext.request.contextPath}/report/radio3g/cell/wk/bhDetails.htm?function=${function}&cellid=${cellid}&bscid=${bscid}&endWeek=${endWeek}&endYear=${endYear}"><span>Báo cáo BH tuần</span></a></li>
...
<div  id="doublescroll">
	<display:table name="${vRpWkCellList}" id="vRpWkCell" requestURI="" pagesize="100" class="simple2" export="true">
	    ...
	    <c:choose>
	    	...
	    	<c:otherwise>
				...
				<display:column property="psr99Drop" decorator="vn.com.vhc.vmsc2.statistics.web.utils.NumberDecorator" titleKey="PSR99_DROP" sortable="true" headerClass="master-header-5"/>
		   		...
		   		<display:column property="csIratHoSr" titleKey="CS_IRAT_HO_SR" sortable="true" headerClass="master-header-6"/>
		   		<display:column property ="psIratHoSr" titleKey="PS_IRAT_HO_SR" sortable="true" headerClass="master-header-6 margin" class="margin"/>
		   		<display:column property ="doKhaDung3g" decorator="vn.com.vhc.vmsc2.statistics.web.utils.NumberDecorator" titleKey="DO_KHA_DUNG_3G" sortable="true"/>
		   	</c:otherwise>
	    </c:choose>
