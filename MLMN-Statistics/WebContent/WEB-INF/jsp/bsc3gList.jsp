...
<title>Danh sách RNC</title>
<content tag="heading">Danh sách RNC</content>
...
<form:form commandName="filter" method="post" action="list.htm">
	...
	<div  class="tableStandar">
		<display:table name="${bsc3gList}" id="bsc3g" requestURI="" pagesize="100" export="false" sort="external" defaultsort="1">
			<display:column class="rightColumnMana" title="STT" > <c:out value="${bsc3g_rowNum}"/> </display:column>
		    ...
		    <display:column property="bscid" title="RNC"/>   
		   ...
		   <c:if test="${checkRoleManager==true}">	
		    <display:column titleKey="Quản lý" media="html">
		    	<a href="form.htm?bscid=${bsc3g.bscid}">Edit</a>&nbsp;
		    	<a href="delete.htm?bscid=${bsc3g.bscid}"
		    	   onclick="return confirm('Bạn có chắc muốn xóa?')" >Delete</a>&nbsp;
		    </display:column>
		    </c:if>
		</display:table>
		<div class="exportlinks">Export options:
			<a href="${pageContext.request.contextPath}/network/rnc/exportExcel.htm?deptid=${filter.dept}&teamid=${filter.team}&subTeam=${filter.subTeam}&vendor=${filter.vendor}&mscid=${filter.mscid}&bscid=${filter.bscid}"><span class="export excel">Excel </span></a>
		</div>
	</div>
</form:form>
