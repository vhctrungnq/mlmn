...
<title><fmt:message key="sidebar.admin.isoInventoryUpload"/></title>
<content tag="heading"><fmt:message key="sidebar.admin.isoInventoryUpload"/></content>
...
<form:form method="post" action="upload.htm" enctype="multipart/form-data" >
	...
	<table class="simple2">
		<tr>
			...
			<td ><input class="button" type="file" name="file" size="90"/>&nbsp;<input type="submit" class="button" name="save" onclick="setAction('import')" value="<fmt:message key="global.button.import"/>"/></td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<ul style="list-style-type: none;">
					...
					&lt;<fmt:message key="isoEquipment.ne"/>&gt;, 
    ...
    <c:if test="${fn:length(failedList) gt 0}">
    	<div id="failed">
    		<div><b>Dữ liệu tài nguyên mạng lưới không hợp lệ  ( ${failNum}/${totalNum} )</b></div>
    		...
    		<div style="max-height: 500px; overflow: auto;">
    			<display:table name="${failedList}" class="simple2" id="item1" requestURI="" export="false" pagesize="700">
						...
						<display:column property="ne" titleKey="isoEquipment.ne" />
						...
						<display:column property="inputStatusName" titleKey="isoEquipment.inputStatus" />
	...
	<c:if test="${fn:length(insertList) gt 0}">
	   	<div id="insertList">
	   		<div><b>Dữ liệu tài nguyên mạng lưới chưa có trong hệ thống  ( ${insertNum}/${totalNum} )</b></div>
	   		
	   		<div style="max-height: 500px; overflow: auto;">
	   			<display:table name="${insertList}" class="simple2" id="item2" requestURI="" export="true" pagesize="700">
						...
						<display:setProperty name="export.xml.filename" value="${exportFileNameInsert}.xml" />
						<display:setProperty name="export.csv.filename" value="${exportFileNameInsert}.csv" />
						<display:setProperty name="export.excel.filename" value="${exportFileNameInsert}.xls" />
				</display:table>
			</div>
		</div>
		<div style="padding-top: 10px;padding-bottom: 5px;">
			<input class="button" type="submit" id="btInsert" name="btInsert" onclick="setAction('insert')" value="<fmt:message key="button.insert"/>"/>
		</div>
	</c:if>
	<c:if test="${fn:length(updateList) gt 0}">
	   	<div id="updateList">
	   		<div><b>Dữ liệu tài nguyên mạng lưới đã tồn tại trong hệ thống  ( ${updateNum}/${totalNum} )</b></div>
	   		
	   		<div style="max-height: 500px; overflow: auto;">
	   			<display:table name="${updateList}" class="simple2" id="item3" requestURI="" export="true" pagesize="700">
						<display:column class="centerColumnMana" title="No." > <c:out value="${item3_rowNum}"/> </display:column>
						<display:column property="deptCode" titleKey="isoEquipment.deptCode" />
						<display:column property="team" titleKey="isoEquipment.team" />
						<display:column property="subTeam" titleKey="isoEquipment.subTeam" />
						<display:column property="province" titleKey="isoEquipment.province" />
						...
						<display:setProperty name="export.excel.include_header" value="true" />
						<display:setProperty name="export.xml.include_header" value="true" />
						<display:setProperty name="export.xml.filename" value="${exportFileNameUpdate}.xml" />
						<display:setProperty name="export.csv.filename" value="${exportFileNameUpdate}.csv" />
						<display:setProperty name="export.excel.filename" value="${exportFileNameUpdate}.xls" />
						
				</display:table>
			</div>
		</div>
		<div style="padding-top: 10px;padding-bottom: 5px;">
			<input class="button" type="submit" id="btUpdate" name="btUpdate" onclick="setAction('update')" value="<fmt:message key="button.update"/>"/>&nbsp;
			<input class="button" type="submit" id="btMove" name="btMove" onclick="setAction('move')" value="<fmt:message key="button.move"/>"/>
		</div>
	</c:if>
	<table>
		<tr>
