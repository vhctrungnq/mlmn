...
<c:choose>
  <c:when test="${inputStatus == 'DEVICE'}">
  	<c:choose>
  		<c:when test="${isoEquipmentAddEdit == 'N'}">
	      	<title><fmt:message key="sidebar.admin.isoEquipmentFormAdd"/></title>
	  		<content tag="heading"><fmt:message key="sidebar.admin.isoEquipmentFormAdd"/></content>
	  	</c:when>
	  	<c:when test="${isoEquipmentAddEdit == 'Y'}">
	      	<title><fmt:message key="sidebar.admin.isoEquipmentFormEdit"/></title>
	  		<content tag="heading"><fmt:message key="sidebar.admin.isoEquipmentFormEdit"/></content>
	  	</c:when>
	 </c:choose> 
  </c:when>
  ...
  <c:otherwise>
  	<c:choose>
  		<c:when test="${isoEquipmentAddEdit == 'N'}">
	      <title><fmt:message key="sidebar.admin.isoEquipmentResourceFormAdd"/></title>
		  <content tag="heading"><fmt:message key="sidebar.admin.isoEquipmentResourceFormAdd"/></content>
	  	</c:when>
	  	<c:when test="${isoEquipmentAddEdit == 'Y'}">
	      <title><fmt:message key="sidebar.admin.isoEquipmentResourceFormEdit"/></title>
		  <content tag="heading"><fmt:message key="sidebar.admin.isoEquipmentResourceFormEdit"/></content>
	  	</c:when>
	 </c:choose>
  </c:otherwise>
</c:choose>
...
<form:form commandName="isoInventory" name="checkform" method="post" action="form.htm" > 
    ...
    <table class="simple2">
      ...
      <tr>
    	   ...
    	   <td><fmt:message key="isoEquipment.ne"/>&nbsp;<font color="red">(*)</font></td>
      ...
      <tr>
        ...
        <td colspan="3">
            ...
            <input class="button" type="button" value="<fmt:message key="global.form.huybo"/>" onClick='window.location="list.htm?inputStatus=${inputStatus}"'>
        </td>
      </tr>
    </table>
</form:form>
...
$('#province').change(function(){
	...
	$.ajax({
	  url: "${pageContext.request.contextPath}/iso/quan-ly-thiet-bi/loadQuanHuyen.htm",
	  beforeSend: function( ) {
		  $('#load').remove();
