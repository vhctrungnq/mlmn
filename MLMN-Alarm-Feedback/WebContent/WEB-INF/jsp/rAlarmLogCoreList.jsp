...
<title><fmt:message key="alarmLogCore.title"/></title>
<content tag="heading"><fmt:message key="alarmLogCore.title"/></content> 	 

<div class="ui-tabs-panel">
<form:form commandName="filter" method="post" action="listCore.htm">
<input type="hidden" name="reloadStr" id="reloadStr" value="${reloadStr}">
	...
	<tr>	
		...
		<td style="width:60px;"><fmt:message key="filter.alarmLog.msc"/></td>
		<td>
			<select name="conMsc" id="conMsc" class="wid30" onchange="xl()">
	           		<c:forEach var="item" items="${conditonList}">
						<c:choose>
			                <c:when test="${item.name == conMsc}">
			...
			<input type ="text"  value="${msc}" name="msc" id="msc" style="width:65%">
		</td>
		
		<td style="width:70px"><fmt:message key="filter.alarmLog.bc"/></td>
		<td>
			<select name="conbc" id="conbc" class="wid30" onchange="xl()">
	           		<c:forEach var="item" items="${conditonList}">
						<c:choose>
			                <c:when test="${item.name == conbc}">
			                    <option value="${item.name}" selected="selected">${item.value}</option>
			                </c:when>
							<c:otherwise>
								<option value="${item.name}">${item.value}</option>
			...
			<input type ="text"  value="${bc}" name="bc" id="bc" style="width:64%">
		</td>	 
	</tr>
	<tr>	
		...
		<td colspan="3">
			<select name="conAlarmName" id="conAlarmName" style="width:22%" onchange="xl()">
	           		<c:forEach var="item" items="${conditonList}">
						<c:choose>
			...
			<input type ="text"  value="${alarmName}" name="alarmName" id="alarmName" style="width:75%">
		</td>
		
		<td><fmt:message key="filter.alarmLog.alarmClass"/></td>
		<td>
			<input type ="text"  value="${alarmClass}" name="alarmClass" id="alarmClass" class="wid30">
		</td>  
 ...
 <div id="doublescroll">
	<display:table name="${alarmList}" class="simple4" id="alarmList" requestURI="" pagesize="50" sort="external" defaultsort="1" export="true">
		...
		<display:column property="mscid" titleKey="filter.alarmLog.msc" sortable="true" sortName="MSCID" class="styleBsc"/>
	  	<display:column property="bc" titleKey="filter.alarmLog.bc" sortable="true" sortName="BC" class="styleBsc"/>
		...
		<display:column property="problemText" titleKey="table.alarmLog.alarmInfo" sortable="true" sortName="PROBLEM_TEXT" class="styleAlarmInfo"/>
...
//reload
 ...
 <script type="text/javascript">
    function DoubleScroll(element) {
        ...
        scrollbar.firstChild.style.width= '200%';
 ...
 <script type="text/javascript">
	$(function() {
		var availableTags = [];
		var i = 0;
		<c:forEach items="${mscidList}" var="listOfNames">
