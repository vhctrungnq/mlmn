...
import vn.com.vhc.vmsc2.statistics.dao.VRpDySgsnRau2gDAO;
 ...
 * 
 * @author Mr. ThangPX
 * @return SGSN Rau 2G Report Level Daily, Hourly, Weekly, Monthly
 */
@Controller
@RequestMapping("/report/core/sgsn-rau-2g-intra-pp-inter/*")
public class SgsnRau2gInterController extends BaseController {
	@Autowired
	private VRpDySgsnRau2gDAO dySgsnDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) String startHour,
		...
		sgsnid = ModelAddtributes.checkSgsn(model, sgsnList, sgsnid, "sgsn-rau-2g-intra-pp-inter");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDySgsnRau2g> dySgsnList = dySgsnDAO.filterHourly("V_RP_HR_SGSN_RAU_2G", sgsnid, s[0], s[1], s[2]);
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDySgsnRau2g> dySgsnList = dySgsnDAO.filterDaily("V_RP_DY_SGSN_RAU_2G", sgsnid, s[0], s[1]);
		model.addAttribute("dySgsnList", dySgsnList);
	...
	public String showReportwk(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startWeek,
		...
		List<VRpDySgsnRau2g> dySgsnList = dySgsnDAO.filterWeekly("V_RP_WK_SGSN_RAU_2G", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnRau2g";
	...
	public String showReportmn(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startMonth,
		...
		List<VRpDySgsnRau2g> dySgsnList = dySgsnDAO.filterMonthly("V_RP_MN_SGSN_RAU_2G", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnRau2g";
