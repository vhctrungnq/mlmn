...
import vn.com.vhc.alarm.utils.log.AL_TelnetLog;
	...
	public static List<Al_TelnetServers> initTaskQueues() {
		...
		try {
			...
			String queryStr = " select ID,VENDOR,NETWORK,NE_TYPE,NE,IP_ADDRESS,TELNET_PORT,TELNET_USER, " +
					...
					" from C_TELNET_SERVERS where upper(STATUS) = 'Y' and upper(NE_TYPE) LIKE '%SGSN%' and upper(MODULE) = 'ALARM'";
	...
	public static void doTask(Al_TelnetServers beanInfo) {  
		...
		try { 
			...
			if(beanInfo.getNeType().equalsIgnoreCase("SGSN")){
				//Telnet lay du lieu Nokia Siemens History SGSN
				/*alTelnetLog = cls.CommandSGSN_History(p_Ne, p_SDate, p_STime, p_IpAddress, p_TelnetUser, p_TelnetPass);*/
				alTelnetLog = cls.CommandNSN_Active_SGSN(p_Ne, p_SDate, p_STime, p_IpAddress, p_TelnetUser, p_TelnetPass);
