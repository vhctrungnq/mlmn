...
import vn.com.vhc.vmsc2.statistics.dao.DyMscMapsigiwkDAO;
import vn.com.vhc.vmsc2.statistics.dao.HrMscMapsigiwkDAO;
...
@Controller
@RequestMapping("/report/core-era/mapsig-perfo/msc/*")
public class MscMapsigiwkController extends BaseController{
	@Autowired
	private HrMscMapsigiwkDAO hrMscmapsigDAO;
	@Autowired
	private DyMscMapsigiwkDAO DyMscmapsigDAO;
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String mscid, @RequestParam(required = false) String startHour,
		...
		if (startDate == null) {
			startDate = endDate;
			    ...
			    List<HrMscMapsigiwk> hrMSCMapsig = hrMscmapsigDAO.filter(startHour,startDate,endHour,endDate,mscid);
				
			    if(endDate.compareTo(startDate) < 0)
				{
					startDate = new Date();
					hrMSCMapsig = hrMscmapsigDAO.filter2(startHour,startDate,endHour,mscid);
				}
				...
				model.addAttribute("hrMSCMapsig", hrMSCMapsig);
	...
	public String showReport(@RequestParam(required = false) String mscid,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				model.addAttribute("endDate", df.format(endDate));
				model.addAttribute("mscid", mscid);
				List<DyMscMapsigiwk> dyMSCMapsig = DyMscmapsigDAO.filter(mscid,df.format(startDate), df.format(endDate));
				model.addAttribute("dyMSCMapsig", dyMSCMapsig);
				MscFilter mscFilter = new MscFilter();
				mscFilter.setType("MSC");
				List<Msc> mscList = mscDAO.filter(mscFilter);
				model.addAttribute("mscList", mscList);
