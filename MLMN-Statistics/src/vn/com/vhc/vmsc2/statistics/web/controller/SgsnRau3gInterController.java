...
import vn.com.vhc.vmsc2.statistics.dao.VRpDySgsnRau3gDAO;
 ...
 */
@Controller
@RequestMapping("/report/core/sgsn-rau-3g-intra-pp-inter/*")
public class SgsnRau3gInterController extends BaseController {
	@Autowired
	private VRpDySgsnRau3gDAO dySgsnDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) String startHour,
		...
		sgsnid = ModelAddtributes.checkSgsn(model, sgsnList, sgsnid, "sgsn-rau-3g-intra-pp-inter");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDySgsnRau3g> dySgsnList = dySgsnDAO.filterHourly("V_RP_HR_SGSN_RAU_3G", sgsnid, s[0], s[1], s[2]);
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDySgsnRau3g> dySgsnList = dySgsnDAO.filterDaily("V_RP_DY_SGSN_RAU_3G", sgsnid, s[0], s[1]);
		model.addAttribute("dySgsnList", dySgsnList);
	...
	public String showReportwk(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startWeek,
		...
		List<VRpDySgsnRau3g> dySgsnList = dySgsnDAO.filterWeekly("V_RP_WK_SGSN_RAU_3G", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnRau3g";
	...
	public String showReportmn(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startMonth,
		...
		List<VRpDySgsnRau3g> dySgsnList = dySgsnDAO.filterMonthly("V_RP_MN_SGSN_RAU_3G", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnRau3g";
