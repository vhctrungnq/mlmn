...
<ul class="ui-tabs-nav">
  <li class=""><a href="${pageContext.request.contextPath}/report/radio/site-qos/hr/details.htm?bscid=${bscid}&siteid=${siteid}&endDate=${endDate}"><span>Báo cáo giờ</span></a></li>
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/site-qos/dy/details.htm?bscid=${bscid}&siteid=${siteid}&endDate=${endDate}"><span>Báo cáo ngày</span></a></li>
...
<display:table name="${vRpDySiteQosDetails}" id="vRpDySiteQosDetail" requestURI="" pagesize="100" class="simple2" export="true">
	    ...
	    <display:column titleKey="BSC" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/bsc/dy/detail.htm?bscid=${vRpDySiteQosDetail.bscid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDySiteQosDetail.day}"/>">${vRpDySiteQosDetail.bscid}</a>
	    </display:column> 
	    <display:column property="siteid" titleKey="SITEID" headerClass="hide" class="hide"/>
	    <display:column titleKey="SITEID" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/site-qos/hr/details.htm?bscid=${vRpDySiteQosDetail.bscid}&siteid=${vRpDySiteQosDetail.siteid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDySiteQosDetail.day}"/>">${vRpDySiteQosDetail.siteid}</a>
	    </display:column>
	...
	$(function() {
		...
		$( '.checkAll' ).live( 'change', function() {
			...
			if($( this ).is( ':checked' )){
				for (var i=6;i<=25;i++)
				{
					$('#vRpDySiteQosDetail td:nth-child('+i+'),#vRpDySiteQosDetail th:nth-child('+i+')').show();
				}
			} else {
				for (var i=6;i<=25;i++)
				{
					$('#vRpDySiteQosDetail td:nth-child('+i+'),#vRpDySiteQosDetail th:nth-child('+i+')').hide();
				}
			}
		});
