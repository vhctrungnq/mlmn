...
<content tag="heading"><fmt:message key="sidebar.district.ibc"/></content>
<ul class="ui-tabs-nav">
	<li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/ibc/district/dy/list.htm"><span>Báo cáo ngày</span></a></li>
	<li class=""><a href="${pageContext.request.contextPath}/report/radio/ibc/district/wk/list.htm"><span>Báo cáo tuần</span></a></li>
...
<div class="ui-tabs-panel">
	...
	<br/>
		...
		<div id="doublescroll">
			<display:table name="${vRpDyDistrictIbc}" id="vRpDyDistrictIbc" requestURI="" pagesize="100" class="simple2" export="true" sort="list">
			    ...
			    <display:column titleKey="PROVINCE" media="html" sortable="true" sortProperty="province">
			    	<a href="${pageContext.request.contextPath}/report/radio/ibc/province/hr/detail.htm?province=${vRpDyDistrictIbc.province}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyDistrictIbc.day}"/>">${vRpDyDistrictIbc.province}</a>
			    </display:column>
...
function xl(){
	...
	$(function() {
		...
		<c:if test="${empty district}">
			$($('#vRpDyDistrictIbc > tbody tr')[0]).before('<c:forEach var="vRpDyRegion" items="${vRpDyRegionList}"><tr class="${vRpDyRegion.region}"><td><fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyRegion.day}"/></td><td>${vRpDyRegion.region}</td><td/><td/><td><fmt:formatNumber pattern="#,###,###,##0.##" value="${vRpDyRegion.sites}"/></td><td><fmt:formatNumber pattern="#,###,###,##0.##" value="${vRpDyRegion.cells}"/></td><td class="margin"><fmt:formatNumber pattern="#,###,###,##0.##" value="${vRpDyRegion.trxs}"/></td><td><fmt:formatNumber pattern="#,###,###,##0.##" value="${vRpDyRegion.tTraf}"/></td><td>${vRpDyRegion.tDrpr}</td><td>${vRpDyRegion.tBlkr}</td><td>${vRpDyRegion.tHoblkr}</td><td>${vRpDyRegion.tEmpdr}</td><td class="margin">${vRpDyRegion.cssr}</td><td><fmt:formatNumber pattern="#,###,###,##0.##" value="${vRpDyRegion.sDef}"/></td><td>${vRpDyRegion.sSsr}</td><td>${vRpDyRegion.sBlkr}</td><td>${vRpDyRegion.sDrpr}</td><td>${vRpDyRegion.tAsr}</td><td class="margin">${vRpDyRegion.halfrate}</td><td>${vRpDyRegion.inHoSucr}</td><td>${vRpDyRegion.ogHoSucr}</td><td class="margin"/><td>${vRpDyRegion.dataload}</td></tr></c:forEach>');
		</c:if>
		/*$($('#vRpDyDistrictIbc > tbody tr')[0]).before('<c:forEach var="vRpDyRegion" items="${vRpDyRegionList}"><tr style="background-color:rgb(83, 142, 213);font-weight: bold;"><td><fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyRegion.day}"/></td><td>${vRpDyRegion.region}</td><td/><td>${vRpDyRegion.sites}</td><td>${vRpDyRegion.cells}</td><td>${vRpDyRegion.trxs}</td><td>${vRpDyRegion.tTraf}</td><td>${vRpDyRegion.halfrate}</td><td>${vRpDyRegion.tDrpr}</td><td>${vRpDyRegion.tBlkr}</td><td>${vRpDyRegion.cssr}</td><td>${vRpDyRegion.sDrpr}</td><td>${vRpDyRegion.sBlkr}</td><td>${vRpDyRegion.inHoSucr}</td><td>${vRpDyRegion.ogHoSucr}</td><td>${vRpDyRegion.dataload}</td></tr></c:forEach>');*/
	});
</script>
<script type="text/javascript">
