...
<content tag="heading">MSC MGW ACCESSIBILITY STS DATA DAILY REPORT</content>
 
<ul class="ui-tabs-nav">
  <li class=""><a href="${pageContext.request.contextPath}/report/core/access-stsdata/hr.htm?mscid=${msc.mscid}&startDate=${startDate}&endDate=${endDate}"><span>Báo cáo giờ</span></a></li>
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/core/access-stsdata/dy.htm?mscid=${msc.mscid}&startDate=${startDate}&endDate=${endDate}"><span>Báo cáo ngày</span></a></li>
</ul>
...
<div  id="doublescroll">
		<display:table name="${DyMscacc}" id="DyMscacc" requestURI="" pagesize="100" class="simple3" export="true">
			<display:column property="mscid" titleKey="SYSTEM" headerClass="hide" class="hide"/>		   
		    <display:column title="SYSTEM" media="html">
		   	 	<a href="${pageContext.request.contextPath}/report/core/access-stsdata/hr.htm?mscid=${DyMscacc.mscid}&startDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${DyMscacc.day}"/>&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${DyMscacc.day}"/>">${DyMscacc.mscid}</a>
		    </display:column>
		    <display:column property ="day" format="{0,date,dd/MM/yyyy}" titleKey="DAY" />
		    <display:column property="aal2Reject" titleKey="Num Aal2 Term Reject"/>
		    <display:column property="ipReject" titleKey="Num IP Term Reject"/>
		    <display:column property="aal2Req" titleKey="Num Aal2 Term Request"/>
		    <display:column property="ipReq" titleKey="Num IP Term Request"/>
		    <display:column property="tdmReject" titleKey="Num TMD Term Reject"/>
		    <display:column property="tdmReq" titleKey="Num TMD Term Request"/>
		    <display:column property="mgwAcc" titleKey="MGW Accessibility (%)"/>
		    <display:column property="gcpCsd" titleKey="Num Gcp Notify Csd Fault Est"/>
		    <display:column property="gcpSpeech" titleKey="Num Gcp Notify Speech Fault Est"/>
		    <display:column property="mgwRetain" titleKey="MGw Service retain (%)"/>
		</display:table>
	</div>
