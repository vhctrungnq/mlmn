...
<content tag="heading">BSC HANDOVER IBC ${bscid} REPORT</content>
<ul class="ui-tabs-nav">
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/ibc/bsc-ho/dy/details.htm?bscid=${bscid}&endDate=${endDate}"><span>Báo cáo ngày</span></a></li>
...
<display:table name="${vRpDyBscHoIbcDetails}" id="vRpDyBscHoIbcDetail" requestURI="" pagesize="100" class="simple2" export="true">
	    ...
	    <display:column titleKey="BSC" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/ibc/bsc-ho/hr/details.htm?bscid=${vRpDyBscHoIbcDetail.bscid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyBscHoIbcDetail.day}"/>">${vRpDyBscHoIbcDetail.bscid}</a>
	    </display:column> 
	...
	$(function() {
		...
		$( '.checkAll' ).live( 'change', function() {
			...
			if($( this ).is( ':checked' )){
				for (var i=4;i<=9;i++)
				{
					$('#vRpDyBscHoIbcDetail td:nth-child('+i+'),#vRpDyBscHoIbcDetail th:nth-child('+i+')').show();
				}
			} else {
				for (var i=4;i<=9;i++)
				{
					$('#vRpDyBscHoIbcDetail td:nth-child('+i+'),#vRpDyBscHoIbcDetail th:nth-child('+i+')').hide();
				}
			}
		});
