...
import vn.com.vhc.vmsc2.statistics.dao.DyMscMgwAccDAO;
import vn.com.vhc.vmsc2.statistics.dao.HrMscMgwAccDAO;
...
@Controller
@RequestMapping("/report/core/access-stsdata/*")
public class MscMgwAccController extends BaseController{
	@Autowired
	private HrMscMgwAccDAO hrMscAccDAO;
	@Autowired
	private DyMscMgwAccDAO dyMscAccDAO;
	@Autowired
	...
	public String Report(@RequestParam(required = true) String mscid, @RequestParam(required = false) String startHour,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				List<HrMscMgwAcc> HrMscacc = hrMscAccDAO.filter(startHour,startDate,endHour,endDate,mscid);
				
				if(endDate.compareTo(startDate) < 0)
				{
					startDate = new Date();
					HrMscacc = hrMscAccDAO.filter2(startHour,startDate,endHour,mscid);
				}
				...
				model.addAttribute("HrMscacc", HrMscacc);
	...
	public String showReport(@RequestParam(required = true) String mscid,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				model.addAttribute("endDate", df.format(endDate));
				model.addAttribute("mscid", mscid);
				List<DyMscMgwAcc> DyMscacc = dyMscAccDAO.filter(mscid,df.format(startDate), df.format(endDate));
				model.addAttribute("DyMscacc", DyMscacc);
				MscFilter h_msc = new MscFilter();
				h_msc.setType("MGW");
				List<Msc> mscList = mscDAO.filter(h_msc);
				model.addAttribute("mscid", mscid);
