...
import vn.com.vhc.vmsc2.statistics.dao.DyMscAuthenDAO;
import vn.com.vhc.vmsc2.statistics.dao.HrMscAuthenDAO;
...
@Controller
@RequestMapping("/report/core-era/authen-cation/*")
public class MscAuthenController extends BaseController{
	@Autowired
	private HrMscAuthenDAO hrMscAUDAO;
	@Autowired
	private DyMscAuthenDAO dyMscAUDAO;
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String mscid, @RequestParam(required = false) String startHour,
			...
			@ModelAttribute("filter") CellFilter filter, ModelMap model, HttpServletRequest request)
			{
				...
				List<HrMscAuthen> HrMscAUTH = hrMscAUDAO.filter(startHour,startDate,endHour,endDate,mscid);
				
				if(endDate.compareTo(startDate) < 0)
				{
					startDate = new Date();
					HrMscAUTH = hrMscAUDAO.filter2(startHour,startDate,endHour,mscid);
				}
				...
				model.addAttribute("HrMscAUTH", HrMscAUTH);
		 ...
		 ModelMap model)
			{
				...
				model.addAttribute("endDate", df.format(endDate));
				model.addAttribute("mscid", mscid);
				List<DyMscAuthen> DyMscAUTH = dyMscAUDAO.filter(df.format(startDate), df.format(endDate),mscid);
				model.addAttribute("DyMscAUTH", DyMscAUTH);
				MscFilter mscFilter = new MscFilter();
				mscFilter.setType("MSC");
				List<Msc> mscList = mscDAO.filter(mscFilter);
				model.addAttribute("mscid", mscid);
