...
import vn.com.vhc.vmsc2.statistics.dao.VRpDySgsnOverviewDAO;
 ...
 * 
 * @author Mr. ThangPX
 * @return SGSN Overview Report Level Daily, Hourly, Weekly, Monthly
 */
@Controller
@RequestMapping("/report/core/sgsn-overview/*")
public class SgsnOverviewController extends BaseController {
	@Autowired
	private VRpDySgsnOverviewDAO dySgsnDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) String startHour,
		...
		sgsnid = ModelAddtributes.checkSgsn(model, sgsnList, sgsnid, "sgsn-overview");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDySgsnOverview> dySgsnList = dySgsnDAO.filterHourly("V_RP_HR_SGSN_OVERVIEW", sgsnid, s[0], s[1], s[2]);
		model.addAttribute("dySgsnList", dySgsnList);
		return "dySgsnOverview";
	}
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDySgsnOverview> dySgsnList = dySgsnDAO.filterDaily("V_RP_DY_SGSN_OVERVIEW", sgsnid, s[0], s[1]);
		model.addAttribute("dySgsnList", dySgsnList);
	...
	public String showReportwk(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startWeek,
		...
		List<VRpDySgsnOverview> dySgsnList = dySgsnDAO.filterWeekly("V_RP_WK_SGSN_OVERVIEW", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnOverview";
	...
	public String showReportmn(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startMonth,
		...
		List<VRpDySgsnOverview> dySgsnList = dySgsnDAO.filterMonthly("V_RP_MN_SGSN_OVERVIEW", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnOverview";
