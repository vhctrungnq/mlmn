...
public class TelnetClientExample {
		...
		try {
			...
			boolean end_loop = false;
				...
				while (true) {
					...
					while((fromServer.equals(" ")) && (end_loop == false));
					try
	                {
						telnetClient.disconnect();
						Writer.close();
	...
	public void Command2G(String p_SeverName, String p_AlarmTypeID,
		...
		try {
			...
			String strFileName = "NOKIA_" + p_SeverName + "_" + "ZEOH.NR."
			...
			try {
				while (true) {
					...
					if (fromServer.contains("ENTER USERNAME") == true) {
						...
						toServer = "ZEOH:" + p_SDate + "," + p_STime + ":NR="
								+ p_AlarmTypeID + ";" + p_Command;
