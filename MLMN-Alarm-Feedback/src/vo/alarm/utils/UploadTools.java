...
public class UploadTools {
	...
	private static DateFormat df_full = new SimpleDateFormat("dd/MM/yyyy");
	private static DateFormat df_full2 = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
	private static DateFormat df_full3 = new SimpleDateFormat("dd/MM/yyyy HH:mm");
	private static DateFormat df_full4 = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
	    	...
	    	while (cells.hasNext()) {
	    		...
	    		case HSSFCell.CELL_TYPE_NUMERIC:
	    			/*try{
    					...
    					String value = df_full2.format(valueDate); 
    					data.add(value);
    				}catch(Exception exp){
    				*/
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static List readXlsFile4(InputStream filePath) throws IOException {
	    	...
	    	while (cells.hasNext()) {
	    		...
	    		case HSSFCell.CELL_TYPE_NUMERIC:
    				...
    				/*	System.out.println("value: "+valueDate);
    					System.out.println("date1: "+df_full2.format(valueDate));
    					System.out.println("date2: "+df_full4.format(valueDate));
    					Calendar cal = Calendar.getInstance();	
    					cal.setTime(valueDate);
    					System.out.println("date3: "+df_full2.format(cal.getTime()));*/
    					data.add(value);
	    			 }
	    			 else
	    			 {
		    ...
		    while (rows.hasNext()) {
		    	...
		    	while (cells.hasNext()) {
		    		...
		    		case HSSFCell.CELL_TYPE_NUMERIC:
			    			if(HSSFDateUtil.isCellDateFormatted(cell)){
		    					Date valueDate = cell.getDateCellValue();
		    					String value = df_full3.format(valueDate); 
		    					data.add(value);
		    				}else{
		    					long value= (long) cell.getNumericCellValue();
		    					data.add(value);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static List readXlsFileForCost(InputStream filePath) throws IOException {
	    ...
	    while (rows.hasNext()) {
	    	...
	    	while (cells.hasNext()) {
	    		...
	    		case HSSFCell.CELL_TYPE_NUMERIC:
	    			...
	    			{
	    				int value=0;
	    				value = Integer.parseInt(String.valueOf(cell.getNumericCellValue()));
	    				data.add(value);
	    			}
	    			catch(Exception exp)
	    			{
	    				...
	    				data.add(df.format(value).replace(",","."));
	    				//data.add(cell.getNumericCellValue());
	    			}
	    		    break;
	    		 default:
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static List readXlsFileForAssets(InputStream file) throws IOException {
	    ...
	    for (int i = 0; i < wb_hssf.getNumberOfSheets(); i++) { 
		    ...
		    while (rows.hasNext()) {
		    	...
		    	//System.out.println("row.getLastCellNum(): " + row.getLastCellNum());
		    	if (sheet1 && first) {
		    		HSSFCell hssfCell1 = row.createCell(row.getLastCellNum() + 1);
		    		hssfCell1.setCellValue("Mã loại thiết bị");
			    	HSSFCell hssfCell2 = row.createCell(row.getLastCellNum() + 2);
			    	hssfCell2.setCellValue("Nhà sản xuất");
		    		...
		    		if (first == false) {
		    			HSSFCell hssfCell1 = row.createCell(row.getLastCellNum() + 1);
		    			hssfCell1.setCellValue(sheetName);
				    	HSSFCell hssfCell2 = row.createCell(row.getLastCellNum() + 2);
				    	hssfCell2.setCellValue(vendor);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static List readXlsFile3(InputStream filePath) throws IOException {
	    ...
	    while (rows.hasNext()) {
	    	...
	    	while (cells.hasNext()) {
	    		...
	    		case HSSFCell.CELL_TYPE_NUMERIC:
	    			try{
    					Date valueDate = cell.getDateCellValue();
    					String value = df_full4.format(valueDate); 
    					data.add(value);
    				}catch(Exception exp){
    					int value= (int) cell.getNumericCellValue();
	    				data.add(value);
