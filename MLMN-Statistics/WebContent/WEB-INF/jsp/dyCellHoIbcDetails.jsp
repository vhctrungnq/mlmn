...
<content tag="heading">CELL HANDOVER STATION IBC REPORT ${cellid}</content>
<ul class="ui-tabs-nav">
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/ell-ho/dy/details.htm?bscid=${bscid}&cellid=${cellid}&endDate=${endDate}"><span>Báo cáo ngày</span></a></li>
...
<display:table name="${vRpDyCellHoIbcDetails}" id="vRpDyCellHoIbcDetail" requestURI="" pagesize="100" class="simple2" export="true">
	    ...
	    <display:column titleKey="BSC" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/ibc/bsc-ho/dy/details.htm?bscid=${vRpDyCellHoIbcDetail.bscid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyCellHoIbcDetail.day}"/>">${vRpDyCellHoIbcDetail.bscid}</a>
	    </display:column>
	    <display:column property="cellid" titleKey="CELL" headerClass="hide" class="hide"/>
	    <display:column titleKey="CELL" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/cell-ho/hr/details.htm?bscid=${vRpDyCellHoIbcDetail.bscid}&cellid=${vRpDyCellHoIbcDetail.cellid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyCellHoIbcDetail.day}"/>">${vRpDyCellHoIbcDetail.cellid}</a>
	    </display:column>
	...
	$(function() {
		...
		$( '.checkAll' ).live( 'change', function() {
			...
			if($( this ).is( ':checked' )){
				for (var i=6;i<=9;i++)
				{
					$('#vRpDyCellHoIbcDetail td:nth-child('+i+'),#vRpDyCellHoIbcDetail th:nth-child('+i+')').show();
				}
			} else {
				for (var i=6;i<=9;i++)
				{
					$('#vRpDyCellHoIbcDetail td:nth-child('+i+'),#vRpDyCellHoIbcDetail th:nth-child('+i+')').hide();
				}
			}
		});
