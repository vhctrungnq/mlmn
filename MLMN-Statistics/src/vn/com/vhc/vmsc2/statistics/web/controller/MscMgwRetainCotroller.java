...
import vn.com.vhc.vmsc2.statistics.dao.DyMscMgwRetainDAO;
import vn.com.vhc.vmsc2.statistics.dao.HrMscMgwRetainDAO;
...
@Controller
@RequestMapping("/report/core/Retaina-stsdata/*")
public class MscMgwRetainCotroller extends BaseController{
	@Autowired
	private HrMscMgwRetainDAO hrMscRetainDAO;
	@Autowired
	private DyMscMgwRetainDAO dyMscRetainDAO;
	@Autowired
	...
	public String Report(@RequestParam(required = true) String mscid, @RequestParam(required = false) String startHour,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				List<HrMscMgwRetain> HrMsretain = hrMscRetainDAO.filter(startHour,startDate,endHour,endDate,mscid);
				
				if(endDate.compareTo(startDate) < 0)
				{
					startDate = new Date();
					HrMsretain = hrMscRetainDAO.filter2(startHour,startDate,endHour,mscid);
				}
				...
				model.addAttribute("HrMsretain", HrMsretain);
				model.addAttribute("hourList", hourList);
				return "hrMSCMgwRetain";
			}
	...
	public String showReport(@RequestParam(required = true) String mscid,
		...
		if (startDate == null) {
			startDate = endDate;
				...
				model.addAttribute("endDate", df.format(endDate));
				model.addAttribute("mscid", mscid);
				List<DyMscMgwRetain> DyMscretain = dyMscRetainDAO.filter(mscid,df.format(startDate), df.format(endDate));
				model.addAttribute("DyMscretain", DyMscretain);
				MscFilter h_msc = new MscFilter();
				h_msc.setType("MGW");
				List<Msc> mscList = mscDAO.filter(h_msc);
				model.addAttribute("mscid", mscid);
				model.addAttribute("mscList", mscList);
				return "dyMscMgwRetain";
			}
}
