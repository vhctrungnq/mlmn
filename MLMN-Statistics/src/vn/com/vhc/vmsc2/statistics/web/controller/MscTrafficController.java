...
import vn.com.vhc.vmsc2.statistics.dao.VRpDyMscTrafficDAO;
 ...
 * 
 * @author Mr. ThangPX
 * @return Traffic Report Level Daily, Hourly, Weekly, Monthly
 */
@Controller
@RequestMapping("/report/core/msc-traffic/*")
public class MscTrafficController extends BaseController {
	@Autowired
	private VRpDyMscTrafficDAO dyMscDAO;
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String mscid, @RequestParam(required = false) String startHour,
		...
		mscid = ModelAddtributes.checkMsc(model, listMsc, mscid, "msc-traffic");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDyMscTraffic> dyMscList = dyMscDAO.filterHourly("V_RP_HR_MSC_TRAFFIC", mscid, s[0], s[1], s[2]);
	...
	public String showReport(@RequestParam(required = false) String mscid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDyMscTraffic> dyMscList = dyMscDAO.filterDaily("V_RP_DY_MSC_TRAFFIC", mscid, s[0], s[1]);
		model.addAttribute("dyMscList", dyMscList);
	...
	public String showReportwk(@RequestParam(required = false) String mscid, @RequestParam(required = false) Integer startWeek,
		...
		List<VRpDyMscTraffic> dyMscList = dyMscDAO.filterWeekly("V_RP_WK_MSC_TRAFFIC", mscid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dyMscList", dyMscList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dyMscList"));
		return "dyMscTraffic";
	...
	public String showReportmn(@RequestParam(required = false) String mscid, @RequestParam(required = false) Integer startMonth,
		...
		List<VRpDyMscTraffic> dyMscList = dyMscDAO.filterMonthly("V_RP_MN_MSC_TRAFFIC", mscid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dyMscList", dyMscList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dyMscList"));
		return "dyMscTraffic";
