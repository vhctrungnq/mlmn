...
import vn.com.vhc.vmsc2.statistics.dao.VRpDySgsnPdpActFail2gDAO;
 ...
 * 
 * @author Mr. ThangPX
 * @return SGSN Pdp Act Fail 2G Report Level Daily, Hourly, Weekly, Monthly
 */
@Controller
@RequestMapping("/report/core/sgsn-pdp-act-fail-2g/*")
public class SgsnPdpActFail2gController extends BaseController {
	@Autowired
	private VRpDySgsnPdpActFail2gDAO dySgsnDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) String startHour,
		...
		sgsnid = ModelAddtributes.checkSgsn(model, sgsnList, sgsnid, "sgsn-pdp-act-fail-2g");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDySgsnPdpActFail2g> dySgsnList = dySgsnDAO.filterHourly("V_RP_HR_SGSN_PDP_ACT_FAIL_2G", sgsnid, s[0], s[1], s[2]);
		model.addAttribute("dySgsnList", dySgsnList);
		return "dySgsnPdpActFail2g";
	}
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDySgsnPdpActFail2g> dySgsnList = dySgsnDAO.filterDaily("V_RP_DY_SGSN_PDP_ACT_FAIL_2G", sgsnid, s[0], s[1]);
		model.addAttribute("dySgsnList", dySgsnList);
	...
	public String showReportwk(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startWeek,
		...
		List<VRpDySgsnPdpActFail2g> dySgsnList = dySgsnDAO.filterWeekly("V_RP_WK_SGSN_PDP_ACT_FAIL_2G", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnPdpActFail2g";
	...
	public String showReportmn(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startMonth,
		...
		List<VRpDySgsnPdpActFail2g> dySgsnList = dySgsnDAO.filterMonthly("V_RP_MN_SGSN_PDP_ACT_FAIL_2G", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnPdpActFail2g";
