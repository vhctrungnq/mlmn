...
import vn.com.vhc.vmsc2.statistics.dao.VRpDyNBscUtilCe3gDAO;
 ...
 */
@Controller
@RequestMapping("/report/core/bsc-util-ce-nsn-3g/*")
public class BscUtilCeNsn3gController extends BaseController {
	@Autowired
	private VRpDyNBscUtilCe3gDAO bscLoadDAO;
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String bscid, @RequestParam(required = false) String startHour,
		...
		bscid = ModelAddtributes.checkRnc(model, listBsc, bscid,"bsc-util-ce-nsn-3g");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDyNBscUtilCe3g> dyBscList = bscLoadDAO.filterHourly("V_RP_HR_N_BSC_UTIL_CE_3G", bscid, "", s[0], s[1], s[2]);
		model.addAttribute("dyBscList", dyBscList);
		model.addAttribute("objectname", "N");
		
		return "bscUtilCeNsn3G";
	}
	...
	public String showReport(@RequestParam(required = false) String bscid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDyNBscUtilCe3g> dyBscList = bscLoadDAO.filterDaily("V_RP_DY_N_BSC_UTIL_CE_3G", bscid, "", s[0], s[1]);
		model.addAttribute("dyBscList", dyBscList);
