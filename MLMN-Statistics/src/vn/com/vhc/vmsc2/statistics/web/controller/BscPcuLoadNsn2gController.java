...
import vn.com.vhc.vmsc2.statistics.dao.VRpDyNBscPcuLoad2gDAO;
 ...
 */
@Controller
@RequestMapping("/report/core/bsc-pcu-load-nsn-2g/*")
public class BscPcuLoadNsn2gController extends BaseController {
	@Autowired
	private VRpDyNBscPcuLoad2gDAO bscLoadDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String bscid, @RequestParam(required = false) String startHour,
		...
		bscid = ModelAddtributes.checkBscid(model, listBsc, bscid,"bsc-pcu-load-nsn-2g");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDyNBscPcuLoad2g> dyBscList = bscLoadDAO.filterHourly("V_RP_HR_N_BSC_PCU_LOAD_2G", bscid, "", s[0], s[1], s[2]);
	...
	public String showReport(@RequestParam(required = false) String bscid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDyNBscPcuLoad2g> dyBscList = bscLoadDAO.filterDaily("V_RP_DY_N_BSC_PCU_LOAD_2G", bscid, "", s[0], s[1]);
		model.addAttribute("dyBscList", dyBscList);
