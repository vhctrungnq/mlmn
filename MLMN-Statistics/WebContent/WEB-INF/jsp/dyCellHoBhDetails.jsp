...
<content tag="heading">CELL HANDOVER REPORT ${cellid}</content>
<ul class="ui-tabs-nav">
  ...
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/cell-ho/dy/bhDetails.htm?bscid=${bscid}&cellid=${cellid}&endDate=${endDate}"><span>Báo cáo BH ngày</span></a></li>
...
<display:table name="${vRpDyCellHoBhDetails}" id="vRpDyCellHoBhDetail" requestURI="" pagesize="100" class="simple2" export="true">
	    <display:column property="day" format="{0,date,dd/MM/yyyy}" titleKey="DAY"/>
	    <display:column titleKey="Busy Hour">
			${vRpDyCellHoBhDetail.bh}:00
		</display:column>
		<display:column property ="bscid" titleKey="BSC" class="hide" headerClass="hide"/>	 
	    <display:column titleKey="BSC" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/bsc-ho/dy/bhDetails.htm?bscid=${vRpDyCellHoBhDetail.bscid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyCellHoBhDetail.day}"/>">${vRpDyCellHoBhDetail.bscid}</a>
	    </display:column>
	    <display:column property="cellid" titleKey="CELL" headerClass="hide" class="hide"/>
	    <display:column titleKey="CELL" media="html">
	    	<a href="${pageContext.request.contextPath}/report/radio/cell-ho/hr/details.htm?bscid=${vRpDyCellHoBhDetail.bscid}&cellid=${vRpDyCellHoBhDetail.cellid}&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${vRpDyCellHoBhDetail.day}"/>">${vRpDyCellHoBhDetail.cellid}</a>
	    </display:column>
	...
	$(function() {	
		$( '.checkAll' ).live( 'change', function() {
			...
			if($( this ).is( ':checked' )){
				for (var i=7;i<=11;i++)
				{
					$('#vRpDyCellHoBhDetail td:nth-child('+i+'),#vRpDyCellHoBhDetail th:nth-child('+i+')').show();
				}
			} else {
				for (var i=7;i<=11;i++)
				{
					$('#vRpDyCellHoBhDetail td:nth-child('+i+'),#vRpDyCellHoBhDetail th:nth-child('+i+')').hide();
				}
			}
		});
