...
import vn.com.vhc.vmsc2.statistics.dao.VRpDySgsnAttFailCauseDAO;
 ...
 * 
 * @author Mr. ThangPX
 * @return SGSN Att Fail Cause Report Level Daily, Hourly, Weekly, Monthly
 */
@Controller
@RequestMapping("/report/core/sgsn-att-fail-cause/*")
public class SgsnAttFailCauseController extends BaseController {
	@Autowired
	private VRpDySgsnAttFailCauseDAO dySgsnDAO; 
	@Autowired
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) String startHour,
		...
		sgsnid = ModelAddtributes.checkSgsn(model, sgsnList, sgsnid, "sgsn-att-fail-cause");
		String[] s = ModelAddtributes.checkHour(model, startDate, startHour, endHour).split(";");
		List<VRpDySgsnAttFailCause> dySgsnList = dySgsnDAO.filterHourly("V_RP_HR_SGSN_ATT_FAIL_CAUSE", sgsnid, s[0], s[1], s[2]);
		model.addAttribute("dySgsnList", dySgsnList);
		return "dySgsnAttFailCause";
	}
	...
	public String showReport(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Date startDate,
		...
		List<VRpDySgsnAttFailCause> dySgsnList = dySgsnDAO.filterDaily("V_RP_DY_SGSN_ATT_FAIL_CAUSE", sgsnid, s[0], s[1]);
		model.addAttribute("dySgsnList", dySgsnList);
	...
	public String showReportwk(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startWeek,
		...
		List<VRpDySgsnAttFailCause> dySgsnList = dySgsnDAO.filterWeekly("V_RP_WK_SGSN_ATT_FAIL_CAUSE", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnAttFailCause";
	...
	public String showReportmn(@RequestParam(required = false) String sgsnid, @RequestParam(required = false) Integer startMonth,
		...
		List<VRpDySgsnAttFailCause> dySgsnList = dySgsnDAO.filterMonthly("V_RP_MN_SGSN_ATT_FAIL_CAUSE", sgsnid, s[0], s[1], s[2], s[3]);
		model.addAttribute("dySgsnList", dySgsnList);
		List<HighlightConfig> highlightConfigList = highlightDao.getByKey("TT");
		model.addAttribute("highlight", Helper.highLightLine(highlightConfigList, "dySgsnList"));
		return "dySgsnAttFailCause";
