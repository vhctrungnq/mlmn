...
<content tag="heading">MSC CP LOAD DAILY REPORT</content>
 
<ul class="ui-tabs-nav">
  <li class=""><a href="${pageContext.request.contextPath}/report/core-era/msc-cpload/hr.htm?mscid=${msc.mscid}&startDate=${startDate}&endDate=${endDate}"><span>Báo cáo giờ</span></a></li>
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/core-era/msc-cpload/dy.htm?mscid=${msc.mscid}&startDate=${startDate}&endDate=${endDate}"><span>Báo cáo ngày</span></a></li>
</ul>
...
<div  id="doublescroll">
		<display:table name="${dyMSCCP}" id="dyMSCCP" requestURI="" pagesize="100" class="simple3" export="true">
		    <display:column property="mscid" titleKey="SYSTEM" headerClass="hide" class="hide"/>
		    <display:column title="SYSTEM" media="html">
		   	 	<a href="${pageContext.request.contextPath}/report/core-era/msc-cpload/hr.htm?mscid=${dyMSCCP.mscid}&startDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${dyMSCCP.day}"/>&endDate=<fmt:formatDate pattern="dd/MM/yyyy" value="${dyMSCCP.day}"/>">${dyMSCCP.mscid}</a>
		    </display:column>
		    <display:column property ="day" format="{0,date,dd/MM/yyyy}" titleKey="Day" />
		    <display:column property="accProcessLoad" titleKey="Accumulated processor load"/>
		    <display:column property="numAcc" titleKey="Num accumulations"/>
		    <display:column property="numOrgCallsFet" titleKey="Num originating calls fetched"/>
		    <display:column property="numIcmCallsFet" titleKey="Num incoming calls fetched"/>
		    <display:column property="numOrgCallsOffer" titleKey="Num originating calls offered"/>
		    <display:column property="numIcmCallsOffer" titleKey="Num of incoming calls offered"/>
		    <display:column property="maxCpLoad" titleKey="Max CPLOAD (%)"/>
		     <display:column property="orgCallsProcessed" titleKey="Org call processed (%)"/>
		    <display:column property="incCallsProcessed" titleKey="Inc call processed (%)"/>
		    <display:column property="callsProcessed" titleKey="Calls processed (%)"/>
		</display:table>
	</div>
<br/>
