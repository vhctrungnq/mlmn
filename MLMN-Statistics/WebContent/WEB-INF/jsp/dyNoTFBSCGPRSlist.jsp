...
<title>NO TRAFFIC BSC GPRS DAILY REPORT</title>
<content tag="heading">NO TRAFFIC BSC GPRS DAILY REPORT</content>
 
<ul class="ui-tabs-nav">
  <li class=""><a href="${pageContext.request.contextPath}/report/radio/bsc-gprs-no-traff/hr/list.htm?bscid=${bsc.bscid}&startDate=${startDate}&startHour=${startHour}&endDate=${endDate}&endHour=${endHour}"><span>Báo cáo giờ</span></a></li>
  <li class="ui-tabs-selected"><a href="${pageContext.request.contextPath}/report/radio/bsc-gprs-no-traff/dy/list.htm?bscid=${bsc.bscid}&startDate=${startDate}&endDate=${endDate}"><span>Báo cáo ngày</span></a></li>
  <li class=""><a href="${pageContext.request.contextPath}/report/radio/bsc-gprs-no-traff/wk/list.htm?bscid=${bsc.bscid}"><span>Báo cáo tuần</span></a></li>
  <li class=""><a href="${pageContext.request.contextPath}/report/radio/bsc-gprs-no-traff/mn/list.htm?bscid=${bsc.bscid}"><span>Báo cáo tháng</span></a></li>
</ul>
<div class="ui-tabs-panel">
	<form:form  method="get" commandName="filter" action="list.htm" name = "frmSample" onSubmit = "return ValidateForm()">
...
<div  style="overflow: auto;">
		<display:table name="${dyBSCGPRS}" id="dyBSCGPRS" requestURI="" pagesize="100" class="simple3" export="true">
...
<script type="text/javascript">
	$(function() {
	 ...
	 $("select#bscid").change(function(){
			$.getJSON("${pageContext.request.contextPath}/ajax/getMsc.htm",{bscid: $(this).val()}, function(j){
				var options = '<option  value="">--Select BSC--</option>';
				for (var i = 0; i < j.length; i++) {
					options += '<option value="' + j[i].bcid + '">' + j[i].bscid + '</option>';
				}
				$("#bscid").html(options);
				$('#bscid option:first').attr('selected', 'selected');
			});
		});
		${highlight}
	}); 
